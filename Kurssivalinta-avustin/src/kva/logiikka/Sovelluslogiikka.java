...
/**
 * Ylin säiliöluokka Kurssivalinta-avustimen sovelluslogiikalle, sisältää
 * {@code Kurssitarjottimen}.
 * <p>
 * {@link kva.logiikka.Kurssitarjotin} luodaan komennoilla
 * {@link #lataaPeriodienNimet(java.lang.String, java.util.function.Consumer, java.util.function.Consumer)}
 * ja
 * {@link #lataaKurssitarjotin(java.util.Collection, java.util.function.Consumer, java.util.function.Consumer)}.
 * Lataamiseen käytetään konstruktoriparametrina annettua
 * {@link kva.logiikka.lataus.KurssitarjottimenLataaja}a.
 *
...
public class Sovelluslogiikka {
     ...
     *
     * @param lataaja {@code KurssitarjottimenLataaja}, joka hakee
     * {@code Kurssitarjottimen} luomiseen tarvittavat tiedot eri käskyllä
     */
     ...
     * <p>
     * Metodia tulee kutsua JavaFX:n sovellussäikeessä. Metodi lähettää nimien
     * lataamisen suoritettavaksi taustasäikeessä, minkä jälkeen ladatut nimet
     * tai mahdollisesti syntyvä poikkeus lähetetään takaisin JavaFX:n
     * sovellussäikeelle.
     * <p>
     * Sovellussäikeelle lähetettävä lista on {@code KurssitarjottimenLataajan}
     * toteutuksesta riippuen toivon mukaan järjestetty: saman oppilaitoksen
     * periodit ovat peräkkäin aikajärjestyksessä.
     *
     * @param tuloksenKasittely Kun periodien nimet on selvitetty, ne lähetetään
     * JavaFX:n sovellussäikeelle kutsumalla metodia {@code accept()}.
     * @param virheenKasittely Jos nimien haku kaatuu poikkeukseen, poikkeus
     * lähetetään JavaFX:n sovellussäikeelle kutsumalla metodia
     * {@code accept()}.
     * @param data tarvittavat tiedot paikasta, josta kurssitarjotin ladataan
     * @throws java.lang.IllegalStateException jos {@link #getTila()} on
     * {@code LADATAAN_PERIODIEN_NIMIA} tai {@code LADATAAN_KURSSITARJOTINTA}
     * @throws java.lang.NullPointerException jos jokin parametreistä on
     * {@code null}
     */
    public void lataaPeriodienNimet(Consumer<List<PeriodinTunniste>> tuloksenKasittely, Consumer<Throwable> virheenKasittely, Object... data) {
        ...
        List<PeriodinTunniste> tulos = null;
        try {
            viesti.setValue("Ladataan periodien nimiä.");
            lataaja.muodostaYhteys(data);
            tulos = lataaja.haePeriodienNimet();
        } catch (Throwable e) {
            onnistui = false;
            viesti.setValue("");
            virheenKasittely.accept(e);
    ...
    /**
     * Lataa {@code Kurssitarjottimen}, joka sisältää saatavilla olevat ryhmät
     * ja moduulit.
     * <p>
     * Metodia tulee kutsua JavaFX:n sovellussäikeessä. Metodi antaa
     * kurssitarjottimen lataamisen taustasäikeen tehtäväksi, minkä jälkeen
     * kurssitarjotin tai mahdollisesti syntyvä poikkeus lähetetään takaisin
     * JavaFX:n sovellussäikeelle.
     *
     * @param valittavat mukaan otettavien periodien tunnisteet, järjestyksellä
     * ei ole väliä
     * @param tuloksenKasittely Kun kurssitarjotin on ladattu, se lähetetään
     * JavaFX:n sovellussäikeelle metodilla {@code accept()}.
     * @param virheenKasittely Jos kurssitarjottimen lataaminen kaatuu
     * poikkeukseen, poikkeus lähetetään JavaFX:n sovellussäikeelle metodilla
     * {@code accept()}.
     * @throws java.lang.IllegalArgumentException jos {@code valittavat}
     * sisältää {@code PeriodinTunnisteita}, joita ei löydy
     * {@link #getPeriodienTunnisteet()}-listalta
     * @throws java.lang.IllegalStateException jos {@link #getTila()} on
     * {@code LUOTU}, {@code LADATAAN_PERIODIEN_NIMIA} tai
     * {@code LADATAAN_KURSSITARJOTINTA}
     * @throws java.lang.NullPointerException jos jokin parametreistä on
     * {@code null}
     */
    public final void lataaKurssitarjotin(Collection<PeriodinTunniste> valittavat, Consumer<Kurssitarjotin> tuloksenKasittely,
        ...
        boolean onnistui = true;
        try {
            viesti.setValue("Valmistellaan kurssitarjottimen latausta.");
            ...
            while (lataaja.onSeuraavaaRyhmaa()) {
                ...
                if (!sijainti.equals(tarkistettava)) {
                    vuoronumero++;
                    viesti.setValue("Ladataan kurssitarjottimia " + vuoronumero 
                            + "/" + periodienMaara + ".");
     ...
     * kurssitarjotinta.
     *
     * @return lista mahdollisten periodien tunnisteista, tai tyhjä lista, jos
     * {@link #getTila()} on {@code LUOTU} tai {@code LADATAAN_PERIODIEN_NIMIÄ}.
     */
     ...
     * kullakin hetkellä.
     *
     * @return lataajan toimintaa kuvaava poikkeusViesti, joka voidaan näyttää
     * käyttöliittymässä.
     */
    ...
    /**
     * Palauttaa {@code Property}-muotoisen viestin, joka kertoo, mitä
     * {@code Sovelluslogiikka} tekee kullakin hetkellä.
     * <p>
     * {@code Propertya} arvoa ei voi muuttaa.
     *
     * @return viestiä kuvaava {@code Property}, jota voi tarkkailla JavaFX:n
     * sovellussäikeessä.
     * @see #getViesti()
     */
     ...
     * <p>
     * {@link kva.logiikka.Sovelluslogiikka} on aina aluksi tilassa
     * {@code LUOTU}. Kun käyttäjä kutsuu metodia
     * {@link #lataaPeriodienNimet(java.lang.String, java.util.function.Consumer, java.util.function.Consumer)},
     * tilaksi vaihtuu {@code LADATAAN_PERIODIEN_NIMIA}. Kun lataus
     * taustasäikeessä saadaan valmiiksi, tilaksi vaihdetaan
     * {@code PERIODIEN_NIMET_LADATTU}, paitsi jos lataus kaatuu poikkeukseen,
     * jolloin lataaja palaa tilaan {@code LUOTU}. Kun käyttäjä periodien nimien
     * lataamisen jälkeen kutsuu metodia
     * {@link #lataaKurssitarjotin(java.util.Collection, java.util.function.Consumer, java.util.function.Consumer)},
     * lataaja siirtyy tilaan {@code LADATAAN_KURSSITARJOTINTA}. Kun lataus on
     * valmis, lataaja siirtyy tilaan {@code KURSSITARJOTIN_LADATTU}, mutta jos
     * se kaatuu poikkeukseen, lataaja palaa tilaan
     * {@code PERIODIEN_NIMET_LADATTU}.
     *
     * @author Väinö Viinikka
    ...
    public enum LatauksenTila {
        ...
        /**
         * Kertoo, että {@code KurssitarjottimenLataajalla} ei ole vielä tehty
         * mitään tai että periodien nimien haku on epäonnistunut.
         *
         */
        LUOTU,
        /**
         * Kertoo, että periodien nimien lataus on parhaillaan käynnissä
         * taustasäikeessä.
         *
         */
        LADATAAN_PERIODIEN_NIMIA,
        /**
         * Kertoo, että periodien nimet on ladattu ja
         * {@code KurssitarjottimenLataaja} on valmis lataamaan itse
         * kurssitarjottimen.
         * <p>
         * LatauksenTila on tässä arvossa myös, mikäli kurssitarjottimen
         * latausta on yritetty, mutta siinä ei ole onnistuttu.
         *
         */
        PERIODIEN_NIMET_LADATTU,
        /**
         * Kertoo, että kurssitarjottimen koostaminen on parhaillaan käynnissä
         * taustasäikeessä.
         *
         */
